...
- `PP / TP / hybrid` 主运行路径不能再默认依赖 `replay bundle / manifest replay` 这类准备物，而要在启动时直接从 `model_path` 构建每个 `stage/rank` 的 `StageState`。
...
- 当前已继续把 concrete runtime modules 的导出面收紧：`pipeline_parallel / hybrid_parallel` 的 `__all__` 只保留 direct runner/loader，`prepare_* / load_*_manifest` 进入 `LEGACY_REPLAY_EXPORTS`；纯 `replay bundle` TP replay 进入 `DEBUG_REPLAY_EXPORTS`。
...
- 当前 `pp / hybrid multimodal generate` runtime-only 主路径真实 smoke 已通过：所有 rank 的 `generated_token_ids` 一致，summary 能证明 `stage0` 负责 frontend + 自己的 stage/shard，non-stage0 只消费 handoff/metadata 且只加载自己的 stage/shard。主路径 `include_runtime_reference=false`，因此不再携带 full reference token 或 `replay bundle`。
- 当前已开始 `TP` 本地分片加载收口：`TextDecoderStageWeightPlan` 与当前代码里的 `load_text_decoder_stage_weight_bundle` 已经明确区分 TP 必须分片读取的 q/k/v/o、MLP 投影参数，以及仍需复制的 embedding、norm、bias、lm_head 等参数；TP 分片模式下缺少必要 `tensor_slices` 会直接报错。文档语义上，这些加载结果属于 `StageState`。
- 当前已确认 `backend=tp|hybrid` direct `StageState` 链路：`backend=tp` 从 `scripts/runtime.py` 进入 `hexgen_core/modules/tensor_parallel.py` 的 standalone TP direct runner，要求单 stage、`tp_degree > 1`、无 PP；`backend=hybrid` 进入 `hybrid_parallel.py`。两者可以共享底层 `StageState` 执行引擎，但 backend 入口和验收边界保持分离。所有 direct `tp_degree > 1` stage 先广播无权重 scaffold，再由每个 rank 用本地 `tp_shard_rank/tp_shard_world_size` materialize 自己的 shard。若 materialize 出来的 `StageState` 不是 `tp_weight_sharded=True`，会直接报错。
- 当前已完成一轮 runtime 命名整理：`tensor_parallel.py` 的主路径公开名收口为 `TensorParallelRunner / run_tensor_parallel_rank / run_stage_state_tp`，无引用的旧 text-specific TP wrapper 已删除；核心包内 import 已优先改为相对路径，`scripts/runtime.py` 因 torchrun 文件直跑入口保留绝对 import。
...
- 已完成：文档术语已切换为主路径使用 `StageState`，旧调试/回放路径继续使用 `replay bundle`。
- 已完成：代码已新增 `StageState` 类型别名和新函数名 wrapper，包括 `build_direct_stage_state`、`materialize_text_stage_state`、`load_stage_state_by_index`、`load_stage_state_for_rank`、`load_stage_state_for_hybrid_rank`；旧 `bundle` 名称继续作为兼容入口保留，但不进入主 `__all__`。
- 已完成：主运行路径内部变量已从 `stage_bundle` 收口到 `stage_state`。`DirectStageStateBuilder` 的 direct materialize 分支、`runtime_text_stage` 的 scaffold/materialize helpers、`execution/stages.py` 的 `forward_text_* / trace_text_*` 入口现在都使用 `StageState` 语义。
- 已完成：`bundle` 这个词只保留在 capture/replay/debug、file-backed reference、仍被测试覆盖的 legacy compatibility、以及底层 weight/layer 兼容命名里；direct package export surface 使用 `StageState` / `compose_layer_state`，不再把主路径叫做 stage bundle。

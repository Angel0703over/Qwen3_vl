...
def load_prefill_bundle(bundle_path: str, device: torch.device, compute_dtype_arg: str) -> tuple[dict, torch.dtype]:
...
def run_direct(args) -> None:
    ...
    bundle, compute_dtype = load_prefill_bundle(args.bundle_path, device, args.compute_dtype)
...
def run_prepare_pp(args) -> None:
    ...
    print(
        f"[prepare-pp] stage0_input_shape={tuple(stage0_bundle['stage_input'].shape)} "
        f"last_output_shape={tuple(last_bundle['stage_output'].shape)}"
    )
    ...
    print(
        f"[prepare-pp] stage0_sanity max_diff={stage0_bundle['sanity_max_diff']} "
        f"mean_diff={stage0_bundle['sanity_mean_diff']}"
    )
    if "hidden_stage_sanity_max_diff" in last_bundle:
        print(
            f"[prepare-pp] last_hidden_sanity max_diff={last_bundle['hidden_stage_sanity_max_diff']} "
            f"mean_diff={last_bundle['hidden_stage_sanity_mean_diff']}"
        )
    if "norm_sanity_max_diff" in last_bundle:
        print(
            f"[prepare-pp] last_norm_sanity max_diff={last_bundle['norm_sanity_max_diff']} "
            f"mean_diff={last_bundle['norm_sanity_mean_diff']}"
        )
    if "logits_sanity_max_diff" in last_bundle:
        print(
            f"[prepare-pp] last_logits_sanity max_diff={last_bundle['logits_sanity_max_diff']} "
            f"mean_diff={last_bundle['logits_sanity_mean_diff']}"
        )
        print(f"[prepare-pp] reference_last_token_topk={summarize_last_token_topk(last_bundle['logits'], args.topk)}")

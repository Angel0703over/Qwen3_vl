#!/usr/bin/env bash
# Multi-node PP multimodal generate smoke wrapper.
...
CASE_ID="${CASE_ID:-pp-mm-generate}"
...
if ! is_uint "${NNODES}" || (( 10#${NNODES} < 2 )); then
  echo "Invalid NNODES=${NNODES}; pure PP smoke requires at least 2 nodes." >&2
  exit 2
fi
...
if ! is_uint "${PP}" || (( 10#${PP} < 2 )); then
  echo "Invalid PP=${PP}; pure PP smoke requires PP >= 2." >&2
  exit 2
fi
if (( 10#${PP} != 10#${NNODES} )); then
  echo "Invalid PP=${PP} for NNODES=${NNODES}; pure PP smoke currently requires PP == NNODES." >&2
  exit 2
fi
...
echo "[smoke] case=${CASE_ID} rank=${NODE_RANK}/${NNODES} pp=${PP} log=${LOG_PATH}"

"""Shared helpers for extracting replayable Qwen3-VL bundles."""
...
from qwen3vl_tp_runtime.models.qwen3vl.execution import forward_decoder_layer, forward_layer_range
from qwen3vl_tp_runtime.models.qwen3vl.functional import build_causal_mask, cast_cpu, resolve_save_dtype
...
def extract_past_key_values(
    past_key_values,
) -> list[tuple[torch.Tensor, torch.Tensor]]:
    if past_key_values is None:
        raise ValueError("past_key_values 不能为空。")
    ...
    if hasattr(past_key_values, "layers"):
        ...
        for layer in past_key_values.layers:
            ...
            if keys is None or values is None:
                raise ValueError("DynamicCache layer 中没有 keys/values。")
            cache_layers.append((keys.detach().clone(), values.detach().clone()))
        return cache_layers
    ...
    if isinstance(past_key_values, (tuple, list)):
        ...
        for layer in past_key_values:
            if not isinstance(layer, (tuple, list)) or len(layer) < 2:
                raise ValueError("legacy past_key_values 的每层都必须至少包含 key/value。")
            cache_layers.append((layer[0].detach().clone(), layer[1].detach().clone()))
        return cache_layers

    raise TypeError(f"不支持的 past_key_values 类型: {type(past_key_values)!r}")

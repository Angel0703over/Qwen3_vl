...
- `modules/`: 并行运行时主入口
  - `tensor_parallel.py`: 纯 TP stage runtime
  - `pipeline_parallel.py`: 纯 PP stage runtime
  - `hybrid_parallel.py`: PP + TP hybrid runtime
- `config.py`: 默认路径、bundle/manifest 常量
...
- `stage.py`: stage bundle view、handoff 构造与 stage replay

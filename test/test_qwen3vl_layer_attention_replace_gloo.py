...
DEFAULT_BUNDLE_PATH = "/mnt/ssd/code/Qwen3_vl/qwen3vl_layer_attention_replace_case.pt"
...
def prepare_bundle(args):
    ...
    def input_ln_pre_hook(_module, module_inputs):
        captured["attn_residual"] = module_inputs[0].detach().clone()
    ...
    def attn_pre_hook(_module, module_args, module_kwargs):
        ...
        captured["attn_input"] = hidden_states.detach().clone()
        if attention_mask is not None:
    ...
    input_ln_pre_handle = input_layernorm.register_forward_pre_hook(input_ln_pre_hook)
    attn_pre_handle = attn_module.register_forward_pre_hook(attn_pre_hook, with_kwargs=True)
    attn_forward_handle = attn_module.register_forward_hook(attn_forward_hook)
    try:
    ...
    required = {"attn_residual", "attn_input", "cos", "sin", "attn_output"}
    if not required.issubset(captured):
        missing = required - set(captured)
        raise RuntimeError(f"没有捕获到目标 attention replace 的必要输入: {missing}")

    attn_residual = captured["attn_residual"].detach().clone()
    ...
    reference_layer_output = attn_residual + attn_output
    ...
    with torch.inference_mode():
        direct_attn_output, _, _, _, _ = attention_forward_from_tensors(
            attn_input,
            attention_mask,
            cos,
            sin,
        ...
        direct_layer_output = attn_residual + direct_attn_output
    ...
    bundle = {
        ...
        "module_name": "layer_attention_replace",
        ...
        "attn_residual": cast_cpu_tensor(attn_residual, save_dtype),
    ...
    print(
        f"[prepare] attn_residual_shape={tuple(bundle['attn_residual'].shape)} "
...
def run_tp(args):
    ...
    attn_residual = bundle["attn_residual"].to(device=device, dtype=compute_dtype)
    attn_input = bundle["attn_input"].to(device=device, dtype=compute_dtype)
    ...
    print(
        f"[config] attn_residual_shape={tuple(attn_residual.shape)} "
        f"attn_input_shape={tuple(attn_input.shape)} layer_output_shape={tuple(reference_layer_output.shape)} "
        f"dtype={attn_input.dtype} bundle_dtype={bundle['save_dtype']} "
    ...
    direct_attn_output, direct_q, direct_k, direct_v, direct_attn_flat = attention_forward_from_tensors(
        attn_input,
        attention_mask,
        cos,
        sin,
    ...
    print(
        f"[direct] rank={rank} direct_layer_attention_replace_vs_reference max_diff={direct_max_diff} "
    ...
    local_q = rms_norm_last_dim(
        local_q_proj.view(*attn_input.shape[:-1], local_q_heads, head_dim),
        q_norm_weight,
        rms_norm_eps,
    ).transpose(1, 2)
    ...
    local_k = rms_norm_last_dim(
        local_k_proj.view(*attn_input.shape[:-1], local_kv_heads, head_dim),
        k_norm_weight,
        rms_norm_eps,
    ).transpose(1, 2)
    ...
    ).view(*attn_input.shape[:-1], local_kv_heads, head_dim).transpose(1, 2)
    ...
    local_attn_flat = local_attn_output.reshape(*attn_input.shape[:-1], -1).contiguous()
    ...
    reduced_output = all_reduce_sum_cpu(local_o_partial, attn_input.device, attn_input.dtype, comm_dtype)
    ...
    tp_attn_output = reduced_output_cpu.to(device=attn_input.device, dtype=attn_input.dtype)
    tp_layer_output = attn_residual + tp_attn_output
    ...
    print(
        f"[layer-attn] rank={rank} local_q_shape={tuple(local_q.shape)} local_k_shape={tuple(local_k.shape)} "
        f"local_v_shape={tuple(local_v.shape)} local_attn_flat_shape={tuple(local_attn_flat.shape)} "
        f"local_o_partial_shape={tuple(local_o_partial.shape)} tp_layer_output_shape={tuple(tp_layer_output.shape)}"
    )
    print(
        f"[layer-attn] rank={rank} q_local_vs_direct max_diff={q_local_vs_direct_max} mean_diff={q_local_vs_direct_mean} "
    ...
    print(
        f"[layer-attn] rank={rank} attn_local_vs_direct max_diff={attn_local_vs_direct_max} "
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3-VL 真实 decoder layer 的 attention replace TP 验证。")
    subparsers = parser.add_subparsers(dest="command", required=True)

    prepare_parser = subparsers.add_parser("prepare", help="从真实 decoder layer 导出 attention replace 样本。")
    ...
    tp_parser = subparsers.add_parser("tp", help="用 gloo 双进程验证 decoder layer 的 attention replace。")
